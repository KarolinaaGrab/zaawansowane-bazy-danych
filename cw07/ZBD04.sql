...
SELECT
    ...
    -- dla wartości inkrementowanych – będzie to zawsze numer bezwzględnej pozycji rekordu w tabeli wynikowej
    -- Funkcja ta, z założenia może być nieciągła – czyli może zostawiać „dziury” w numeracji
    RANK() OVER (  
        PARTITION BY p.product_category
        ORDER BY s.price DESC
    ) AS ranking_ceny,
    
    -- numer wiersza (zawsze kolejne liczby bez pomijania)
    ROW_NUMBER() OVER ( 
        PARTITION BY p.product_category
        ORDER BY s.price DESC
    ) AS number_wiersza_od_ceny,
    
    --  jest funkcją ciągłą, więc jeśli inkrementuje wartości, robi to bez względu na faktyczną pozycję w rankingu, nadaje dokładnie kolejny numer pozycji.
    DENSE_RANK() OVER ( 
        PARTITION BY p.product_category
        ORDER BY s.price DESC
    ) AS ranking_gesty
...
SELECT
    ...
    p.product_name,
    s.sale_date,
    (s.price * s.quantity) as wartosc_zamowienia,
    
    -- rosnąca wartość sprzedaży dla pracownika według daty
    SUM(s.quantity * s.price) OVER (
        PARTITION BY s.employee_id
        ORDER BY s.sale_date
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
    ) AS narastajaca_wartosc_sprzedazy,
